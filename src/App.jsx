...
const STORAGE_KEY = "lawyer-crm-data";

const defaultData = {
  clients: [
    { id: "c1", name: "Maria Santos", email: "maria@example.com", phone: "(555) 234-1001", company: "Santos Holdings", notes: "Referred by Judge Thompson", createdAt: "2025-11-10" },
    { id: "c2", name: "James Whitfield", email: "james.w@whitfield.co", phone: "(555) 234-1002", company: "Whitfield & Co", notes: "Corporate restructuring client", createdAt: "2025-12-01" },
    { id: "c3", name: "Anika Patel", email: "anika@patelgroup.com", phone: "(555) 234-1003", company: "Patel Group LLC", notes: "Immigration matters", createdAt: "2026-01-15" },
  ],
  cases: [
    { id: "cs1", clientId: "c1", title: "Santos v. Metro Properties", caseNumber: "2025-CV-4412", type: "Civil Litigation", status: "Active", priority: "High", description: "Property dispute regarding commercial lease terms", openDate: "2025-11-15", dueDate: "2026-06-01" },
    { id: "cs2", clientId: "c2", title: "Whitfield Corp Restructuring", caseNumber: "2026-CR-0088", type: "Corporate", status: "Active", priority: "Medium", description: "Chapter 11 restructuring advisory", openDate: "2026-01-10", dueDate: "2026-09-30" },
    { id: "cs3", clientId: "c3", title: "Patel Immigration Filing", caseNumber: "2026-IM-0201", type: "Immigration", status: "Pending", priority: "High", description: "EB-1A extraordinary ability petition", openDate: "2026-02-01", dueDate: "2026-05-15" },
    { id: "cs4", clientId: "c1", title: "Santos Estate Planning", caseNumber: "2026-EP-0034", type: "Estate", status: "Closed", priority: "Low", description: "Trust and will preparation", openDate: "2025-09-01", dueDate: "2025-12-15" },
  ],
  tasks: [
    { id: "t1", caseId: "cs1", title: "File motion to compel discovery", status: "In Progress", priority: "High", dueDate: "2026-04-10", notes: "Need to include email exhibits" },
    { id: "t2", caseId: "cs1", title: "Prepare witness deposition outline", status: "Todo", priority: "Medium", dueDate: "2026-04-18", notes: "" },
    { id: "t3", caseId: "cs2", title: "Review creditor claims list", status: "Done", priority: "High", dueDate: "2026-03-25", notes: "Completed review, 3 disputed claims" },
    { id: "t4", caseId: "cs3", title: "Compile recommendation letters", status: "In Progress", priority: "High", dueDate: "2026-04-08", notes: "Need 2 more from academic references" },
    { id: "t5", caseId: "cs2", title: "Draft disclosure statement", status: "Todo", priority: "Medium", dueDate: "2026-04-22", notes: "" },
    { id: "t6", caseId: "cs3", title: "Submit I-140 petition", status: "Todo", priority: "High", dueDate: "2026-05-01", notes: "Awaiting final documents" },
  ],
  billing: [
    { id: "b1", caseId: "cs1", description: "Initial case review & strategy", hours: 4.5, rate: 350, date: "2025-11-20", status: "Paid" },
    { id: "b2", caseId: "cs1", description: "Motion drafting", hours: 6, rate: 350, date: "2026-03-15", status: "Invoiced" },
    { id: "b3", caseId: "cs2", description: "Restructuring consultation", hours: 8, rate: 400, date: "2026-01-20", status: "Paid" },
    { id: "b4", caseId: "cs2", description: "Creditor negotiation calls", hours: 3, rate: 400, date: "2026-03-10", status: "Invoiced" },
    { id: "b5", caseId: "cs3", description: "Immigration petition preparation", hours: 10, rate: 300, date: "2026-02-15", status: "Draft" },
    { id: "b6", caseId: "cs1", description: "Deposition preparation", hours: 5, rate: 350, date: "2026-04-01", status: "Draft" },
  ],
  events: [
    { id: "e1", caseId: "cs1", title: "Santos Deposition", date: "2026-04-14", time: "09:00", duration: 180, type: "Deposition", location: "Office - Conference Room A" },
    { id: "e2", caseId: "cs2", title: "Creditor Committee Meeting", date: "2026-04-10", time: "14:00", duration: 120, type: "Meeting", location: "Whitfield Corp HQ" },
    { id: "e3", caseId: "cs3", title: "USCIS Filing Deadline", date: "2026-05-01", time: "17:00", duration: 0, type: "Deadline", location: "" },
    { id: "e4", caseId: "cs1", title: "Court Hearing - Discovery Motion", date: "2026-04-22", time: "10:30", duration: 60, type: "Court", location: "District Court, Rm 4B" },
    { id: "e5", caseId: "cs2", title: "Client Check-in Call", date: "2026-04-07", time: "11:00", duration: 30, type: "Meeting", location: "Zoom" },
  ],
};
...
.task-info h4.done-text { text-decoration: line-through; color: var(--text-muted); }
...
@media (max-width: 900px) {
  ...
  .sidebar-brand h1 span, .sidebar-brand p, .nav-item span, .nav-badge { display: none; }
...
function Dashboard({ data, setPage }) {
  ...
  const totalBilled = data.billing.reduce((s, b) => s + b.hours * b.rate, 0);
  const unpaid = data.billing.filter((b) => b.status !== "Paid").reduce((s, b) => s + b.hours * b.rate, 0);
    ...
    <div>
      <div className="stats-grid">
        <div className="stat-card">
          ...
          <div className="sub">{data.cases.filter((c) => c.status === "Closed").length} closed</div>
        </div>
        <div className="stat-card">
          <div className="label">Open Tasks</div>
          <div className="value">{openTasks.length}</div>
          <div className="sub" style={{ color: urgentTasks.length > 0 ? "var(--red)" : undefined }}>
            {urgentTasks.length} urgent
          </div>
        </div>
        <div className="stat-card">
          <div className="label">Total Billed</div>
          <div className="value">{fmtMoney(totalBilled)}</div>
          <div className="sub">{fmtMoney(unpaid)} outstanding</div>
        </div>
        <div className="stat-card">
          <div className="label">Clients</div>
          <div className="value">{data.clients.length}</div>
          <div className="sub">{data.events.length} events scheduled</div>
        </div>
      </div>
      <div className="dash-grid">
        <div className="card">
          <div className="card-header">
            ...
            <button className="btn btn-sm" onClick={() => setPage("calendar")}>{Icons.chevron}</button>
          </div>
              ...
              return (
                <div className="upcoming-item" key={ev.id}>
                  <div className="upcoming-date">
                    ...
                    <div className="month">{d.toLocaleString("en-US", { month: "short" })}</div>
                  </div>
                  <div className="upcoming-info">
                    <h4>{ev.title}</h4>
                    <p>{ev.time} · {ev.type}{ev.location ? ` · ${ev.location}` : ""}</p>
                  </div>
                </div>
              );
            })}
        ...
        <div className="card">
          <div className="card-header">
            ...
            <button className="btn btn-sm" onClick={() => setPage("tasks")}>{Icons.chevron}</button>
          </div>
          <div className="card-body">
            {sortedTasks.map((t) => {
              const cs = data.cases.find((c) => c.id === t.caseId);
              return (
                <div className="task-item" key={t.id}>
                  <span className={`priority-dot ${t.priority}`} />
                  <div className="task-info">
                    <h4>{t.title}</h4>
                    <p>{cs?.caseNumber} · Due {t.dueDate}</p>
                  </div>
                  <span className={`badge ${t.status === "In Progress" ? "badge-blue" : "badge-amber"}`}>{t.status}</span>
                </div>
              );
            })}
          </div>
...
function ClientsPage({ data, setData }) {
  ...
  const save = () => {
    if (!form.name) return;
    if (modal === "new") {
      setData((d) => ({ ...d, clients: [...d.clients, { ...form, id: "c" + uid(), createdAt: new Date().toISOString().split("T")[0] }] }));
    } else {
      setData((d) => ({ ...d, clients: d.clients.map((c) => (c.id === form.id ? form : c)) }));
    }
    setModal(null);
  };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
        <button className="btn btn-primary" onClick={openNew}>{Icons.plus}<span>Add Client</span></button>
      </div>
      <div className="card">
                ...
                <tr key={c.id}>
                  ...
                  <td>
                    <div className="actions-cell">
                      <button className="btn-icon" onClick={() => openEdit(c)}>{Icons.edit}</button>
                      <button className="btn-icon btn-danger" onClick={() => remove(c.id)}>{Icons.trash}</button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {modal && (
        <Modal title={modal === "new" ? "New Client" : "Edit Client"} onClose={() => setModal(null)} onSave={save}>
...
function CasesPage({ data, setData }) {
  ...
  const openNew = () => {
    setForm({ clientId: data.clients[0]?.id || "", title: "", caseNumber: "", type: "Civil Litigation", status: "Active", priority: "Medium", description: "", openDate: new Date().toISOString().split("T")[0], dueDate: "" });
    setModal("new");
  };
  ...
  const save = () => {
    ...
    if (modal === "new") {
      setData((d) => ({ ...d, cases: [...d.cases, { ...form, id: "cs" + uid() }] }));
    } else {
      setData((d) => ({ ...d, cases: d.cases.map((c) => (c.id === form.id ? form : c)) }));
    }
    setModal(null);
  };

  const remove = (id) => setData((d) => ({ ...d, cases: d.cases.filter((c) => c.id !== id) }));

  const statusBadge = (s) => {
    const m = { Active: "badge-green", Pending: "badge-amber", Closed: "badge-blue" };
    return <span className={`badge ${m[s] || "badge-purple"}`}>{s}</span>;
  };

  return (
    <div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
        <button className="btn btn-primary" onClick={openNew}>{Icons.plus}<span>New Case</span></button>
      </div>
      <div className="card">
        <div className="table-wrap">
          <table>
      ...
      {modal && (
        <Modal title={modal === "new" ? "New Case" : "Edit Case"} onClose={() => setModal(null)} onSave={save}>
          ...
          <div className="form-row">
            <div className="form-group"><label>Type</label>
              <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>
                {["Civil Litigation", "Corporate", "Immigration", "Estate", "Criminal", "Family", "Real Estate", "IP", "Tax", "Other"].map((t) => <option key={t}>{t}</option>)}
              </select>
            </div>
...
function TasksPage({ data, setData }) {
  ...
  const openNew = () => {
    setForm({ caseId: data.cases[0]?.id || "", title: "", status: "Todo", priority: "Medium", dueDate: "", notes: "" });
    setModal("new");
  };
  ...
  const save = () => {
    if (!form.title) return;
    if (modal === "new") {
      setData((d) => ({ ...d, tasks: [...d.tasks, { ...form, id: "t" + uid() }] }));
    } else {
  ...
  const toggleDone = (id) => {
    setData((d) => ({
      ...d,
      tasks: d.tasks.map((t) => t.id === id ? { ...t, status: t.status === "Done" ? "Todo" : "Done" } : t),
    }));
  };
  ...
  return (
    <div>
          ...
          <table>
            ...
            <tbody>
              {filtered.map((t) => {
                ...
                return (
                  <tr key={t.id}>
                    <td><div className={`task-checkbox ${t.status === "Done" ? "done" : ""}`} onClick={() => toggleDone(t.id)} /></td>
                    <td><span className={t.status === "Done" ? "done-text" : ""} style={t.status === "Done" ? { textDecoration: "line-through", color: "var(--text-muted)" } : { fontWeight: 500 }}>{t.title}</span></td>
                    ...
                    <td>
                      <div className="actions-cell">
                        ...
                        <button className="btn-icon btn-danger" onClick={() => remove(t.id)}>{Icons.trash}</button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      {modal && (
        <Modal title={modal === "new" ? "New Task" : "Edit Task"} onClose={() => setModal(null)} onSave={save}>
          <div className="form-group"><label>Title</label><input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></div>
          <div className="form-row">
            <div className="form-group"><label>Case</label>
              <select value={form.caseId} onChange={(e) => setForm({ ...form, caseId: e.target.value })}>
                {data.cases.map((c) => <option key={c.id} value={c.id}>{c.caseNumber} — {c.title}</option>)}
              </select>
            </div>
            <div className="form-group"><label>Priority</label>
              <select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value })}>
...
// ─── Billing Page ───
function BillingPage({ data, setData }) {
  const [modal, setModal] = useState(null);
  const [form, setForm] = useState({});

  const openNew = () => {
    setForm({ caseId: data.cases[0]?.id || "", description: "", hours: "", rate: 350, date: new Date().toISOString().split("T")[0], status: "Draft" });
    setModal("new");
  };
  ...
  const save = () => {
    ...
    const entry = { ...form, hours: parseFloat(form.hours) || 0, rate: parseFloat(form.rate) || 0 };
    if (modal === "new") {
      setData((d) => ({ ...d, billing: [...d.billing, { ...entry, id: "b" + uid() }] }));
    } else {
      setData((d) => ({ ...d, billing: d.billing.map((b) => (b.id === entry.id ? entry : b)) }));
    }
    setModal(null);
  };

  const remove = (id) => setData((d) => ({ ...d, billing: d.billing.filter((b) => b.id !== id) }));

  const fmtMoney = (n) => "$" + n.toLocaleString("en-US", { minimumFractionDigits: 2 });

  const totalByStatus = (s) => data.billing.filter((b) => b.status === s).reduce((sum, b) => sum + b.hours * b.rate, 0);

  return (
    <div>
      <div className="stats-grid" style={{ gridTemplateColumns: "repeat(3, 1fr)", marginBottom: 20 }}>
        <div className="stat-card"><div className="label">Paid</div><div className="value" style={{ color: "var(--green)" }}>{fmtMoney(totalByStatus("Paid"))}</div></div>
        <div className="stat-card"><div className="label">Invoiced</div><div className="value" style={{ color: "var(--amber)" }}>{fmtMoney(totalByStatus("Invoiced"))}</div></div>
        <div className="stat-card"><div className="label">Draft</div><div className="value" style={{ color: "var(--text-secondary)" }}>{fmtMoney(totalByStatus("Draft"))}</div></div>
      </div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 16 }}>
        <button className="btn btn-primary" onClick={openNew}>{Icons.plus}<span>Add Entry</span></button>
      </div>
      <div className="card">
        <div className="table-wrap">
          <table>
            ...
            <tbody>
              {[...data.billing].sort((a, b) => b.date.localeCompare(a.date)).map((b) => {
                const cs = data.cases.find((c) => c.id === b.caseId);
                return (
                  <tr key={b.id}>
                    ...
                    <td>{b.hours}h</td>
                    <td>{fmtMoney(b.rate)}/hr</td>
                    <td style={{ fontWeight: 600 }}>{fmtMoney(b.hours * b.rate)}</td>
                    <td><span className={`badge ${b.status === "Paid" ? "badge-green" : b.status === "Invoiced" ? "badge-amber" : "badge-blue"}`}>{b.status}</span></td>
                    <td>
                      <div className="actions-cell">
                        <button className="btn-icon" onClick={() => openEdit(b)}>{Icons.edit}</button>
                        <button className="btn-icon btn-danger" onClick={() => remove(b.id)}>{Icons.trash}</button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      {modal && (
        <Modal title={modal === "new" ? "New Billing Entry" : "Edit Entry"} onClose={() => setModal(null)} onSave={save}>
          ...
          <div className="form-row">
            <div className="form-group"><label>Hours</label><input type="number" step="0.5" value={form.hours} onChange={(e) => setForm({ ...form, hours: e.target.value })} /></div>
            <div className="form-group"><label>Rate ($/hr)</label><input type="number" value={form.rate} onChange={(e) => setForm({ ...form, rate: e.target.value })} /></div>
          </div>
          <div className="form-group"><label>Status</label>
            <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>
              {["Draft", "Invoiced", "Paid"].map((s) => <option key={s}>{s}</option>)}
            </select>
          </div>
        </Modal>
      )}
...
function CalendarPage({ data, setData }) {
  ...
  const openNew = () => {
    setForm({ caseId: data.cases[0]?.id || "", title: "", date: new Date().toISOString().split("T")[0], time: "09:00", duration: 60, type: "Meeting", location: "" });
    setModal("new");
  };

  const save = () => {
    if (!form.title) return;
    const entry = { ...form, duration: parseInt(form.duration) || 0 };
    if (modal === "new") {
      setData((d) => ({ ...d, events: [...d.events, { ...entry, id: "e" + uid() }] }));
    } else {
      setData((d) => ({ ...d, events: d.events.map((e) => (e.id === entry.id ? entry : e)) }));
    }
    setModal(null);
  };
    ...
    <div>
      ...
      <div className="cal-grid">
        ...
        {cells.map((cell, i) => {
          ...
          return (
            <div key={i} className={`cal-cell ${!cell.current ? "other-month" : ""} ${isToday ? "today" : ""}`}>
              <div className="day-num">{cell.day}</div>
              {dayEvents.map((ev) => (
                <div key={ev.id} className={`cal-event ${ev.type}`} title={`${ev.title} — ${ev.time}`} onClick={() => { setForm({ ...ev }); setModal("edit"); }}>
                  {ev.time} {ev.title}
                </div>
              ))}
            </div>
      ...
      {modal && (
        <Modal title={modal === "new" ? "New Event" : "Edit Event"} onClose={() => setModal(null)} onSave={save}>
          <div className="form-group"><label>Title</label><input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></div>
          <div className="form-row">
            <div className="form-group"><label>Date</label><input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} /></div>
          ...
          <div className="form-row">
            <div className="form-group"><label>Type</label>
              <select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>
                {["Meeting", "Court", "Deadline", "Deposition", "Other"].map((t) => <option key={t}>{t}</option>)}
              </select>
            </div>
...
export default function App() {
  ...
  const pages = [
    ...
    { key: "clients", label: "Clients", icon: Icons.clients, count: data.clients.length },
    { key: "cases", label: "Cases", icon: Icons.cases, count: data.cases.filter((c) => c.status !== "Closed").length },
    { key: "tasks", label: "Tasks", icon: Icons.tasks, count: data.tasks.filter((t) => t.status !== "Done").length },
      ...
      <div className="app">
        <aside className="sidebar">
          <div className="sidebar-brand">
            <h1>
              <span className="brand-icon">{Icons.scale}</span>
              <span>LexCRM</span>
            </h1>
          ...
          <nav className="sidebar-nav">
            {pages.map((p) => (
              <button key={p.key} className={`nav-item ${page === p.key ? "active" : ""}`} onClick={() => setPage(p.key)}>
                {p.icon}
                <span>{p.label}</span>
                {p.count !== undefined && <span className="nav-badge">{p.count}</span>}
              </button>
            ))}
          </nav>
        </aside>
        <div className="main">
          ...
          <div className="content">
            ...
            {page === "clients" && <ClientsPage data={data} setData={setData} />}
            {page === "cases" && <CasesPage data={data} setData={setData} />}
            {page === "tasks" && <TasksPage data={data} setData={setData} />}
            {page === "billing" && <BillingPage data={data} setData={setData} />}
